...
exampleSurfaceR2 <- function (temp, pH, noise = 0, seed = NULL) {
...
exampleSurfaceR2.2pks <- function (temp, pH, noise = 0, seed = NULL) {
  if (length(temp) != length(pH)) stop('Vector parameters x and y must have same length')
  if (!missing(seed)) set.seed(seed)
  if (any(any(temp > 365), any(temp < 278), any(pH > 14), any(pH < 0))) return(-1)
...
exampleSurfaceR3 <- function (temp, pH, Conc, noise = 0, seed = NULL) {
