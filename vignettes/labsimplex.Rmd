...
## Using the package
After the variables to optimize had been selected, the values that will be evaluated at the beginning of the experimentation and the initial stepsize for each variable are defined, the experimenter will be able to generate the starting simplex. The optimization using labsimplex package involves the next steps that will be further discussed later:
...
3. Perform the experiments defined in the vertices of the initial vertex and evaluate their quality function.
...
7. Perform the new experiment and calculate the quality function of last simplex.
8. Repeat steps 4 to 7 until optimization is finished.

If the experiments do not take long the quality function can be quickly obtained and the process could be optimized in a single R session. In this case steps that include exporting and importing files are not necessary. The simplex coordinates can be plotted (or be projected for higher dimension simplexes) before each new vertex is generated.
...
New vertices are generated using the quality function values for each vertex of previous simplex, the optimization criteria and the algorithm to use. The function used is `generateVertex()`. Suppose the vertex 1 to 4 had quality functions of 65, 72, 54 and 78 respectively. In this example the fixed size simplex algorithm will be used and the purpose of the optimization is maximization. Those are the default settings of the `generateVertex()` function so it will not specified. The vertices quality function for the first simplex must be supplied in ascendant order. The following generation processes will only need the quality function of the last generated vertex. 
